...
a>0;
a--){try{var f=new ActiveXObject("ShockwaveFlash.ShockwaveFlash."+a);
return a
}catch(d){}}}return c
};
gadgets.flash.swfContainerId_=0;
gadgets.flash.embedFlash=function(h,p,n,d){switch(typeof p){case"string":p=document.getElementById(p);
case"object":if(p&&(typeof p.innerHTML==="string")){break
}default:return false
}switch(typeof d){case"undefined":d={};
case"object":break;
default:return false
}if(h.indexOf("//")==0){h=document.location.protocol+h
}var k=gadgets.flash.getMajorVersion();
if(k){var q=parseInt(n,10);
if(isNaN(q)){q=0
}if(k>=q){if(d.width===void 0){d.width="100%"
}if(d.height===void 0){d.height="100%"
}if(typeof d.base!=="string"){var o=document.createElement("a");
o.href=h;
d.base=o.href.match(/^(.*\/)[^\/]*$/)[1]
}if(typeof d.wmode!=="string"){d.wmode="opaque"
}while(!d.id){var e="swfContainer"+gadgets.flash.swfContainerId_++;
if(!document.getElementById(e)){d.id=e
}}var g;
if(navigator.plugins&&navigator.mimeTypes&&navigator.mimeTypes.length){d.type="application/x-shockwave-flash";
d.src=h;
g=document.createElement("embed");
for(var c in d){if(!/^swf_/.test(c)&&!/___$/.test(c)){g.setAttribute(c,d[c])
}}p.innerHTML="";
p.appendChild(g);
return true
}else{var j=function(a){return !/["<>]/.test(a)
};
d.movie=h;
var l={width:d.width,height:d.height,classid:"clsid:D27CDB6E-AE6D-11CF-96B8-444553540000"};
if(d.id){l.id=d.id
}var i="<object";
for(var m in l){if(!/___$/.test(m)&&j(m)&&j(l[m])){i+=" "+m+'="'+l[m]+'"'
}}i+=">";
for(var b in d){var f=document.createElement("param");
if(!/^swf_/.test(b)&&!l[b]&&!/___$/.test(b)&&j(b)&&j(d[b])){i+='<param name="'+b+'" value="'+d[b]+'" />'
}}i+="</object>"
}p.innerHTML=i;
return true
}}return false
...
(function(){var j=window['___jsl']=window['___jsl']||{};j['l']=(j['l']||[]).concat(['core','flash','setprefs']);})();(function(){var j=window['___jsl']=window['___jsl']||{};if(j['c']){j['c']();delete j['c'];}})();