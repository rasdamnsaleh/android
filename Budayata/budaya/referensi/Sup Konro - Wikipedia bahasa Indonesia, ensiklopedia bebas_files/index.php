...
window.pg || $(document).ready( function($) {
    ...
    if ( wgCanonicalNamespace === '' || wgCanonicalNamespace === 'Project' || wgCanonicalNamespace === 'Help' ) {
        ...
        if( settings[0] == 0 ) {
            ...
            footer.append($("<li>").append($("<a>").text("Aktifkan Reference Tooltips").attr("href","javascript:(function(){})()").click(function(){toggleRT(1)})));
            return;
        }
            ...
            function openSettingsMenu(){
                    if( settingsMenu ) {
                            ...
                            settingsMenu = $("<form>").append(
                                    $("<button>").css("width","100%").text("Nonaktifkan Reference Tooltips").button().click(function(){toggleRT(0)}),
                                    $("<br>"),
                                    $("<small>").text("Ketika dinonaktifkan, Reference Tooltips dapat diaktifkan kembali menggunakan pranala di bawahan halaman."),
                                    $("<hr>"),
                                    $("<label>").text("Waktu sebelum tooltip muncul (dalam milidetik): ").append($("<input>").attr({"type":"number","value":settings[1],step:50,min:0,max:5000})),
                                    $("<br>"),
                                    $("<span>").text("Tooltip diaktifkan oleh:"),
                                    $("<label>").append(
                            ...
                            ).submit(function(e){e.preventDefault()}).dialog({modal:true,width:500,title:"Opsi Reference Tooltips",buttons:{"Simpan setelan":function(){
            ...
            $(this)[ isTouchscreen ? 'click' : 'hover' ](function( e ){
                    ...
                    showTimer = setTimeout( function() {
                            ...
                            if(!tooltipNode){
                                    ...
                                    $( tooltipNode.firstChild.insertBefore( document.createElement( "span" ), tooltipNode.firstChild.firstChild ) ).addClass("RTsettings").attr("title", "Setelan Tooltip").click(function(){
